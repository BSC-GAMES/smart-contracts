...
interface IyBSCGToken is IERC20 {
...
contract SwapFeeReward is Ownable{
    ...
    uint256 public maxMiningAmount  = 250000000 * 1e18;
    uint256 public maxMiningInPhase = 10000 * 1e18;
    uint public currentPhase = 1;
    uint256 public totalMined = 0;
    IyBSCGToken public yBSCGToken;
    ...
    constructor(
        ...
        yBSCGToken = _yBSCGToken;
        oracle = _Oracle;
        targetToken = _targetToken;
    }

    function sortTokens(address tokenA, address tokenB) public pure returns (address token0, address token1) {
        require(tokenA != tokenB, 'PancakeSwapFactory: IDENTICAL_ADDRESSES');
        (token0, token1) = tokenA < tokenB ? (tokenA, tokenB) : (tokenB, tokenA);
        require(token0 != address(0), 'PancakeSwapFactory: ZERO_ADDRESS');
    }
    ...
    function getSwapFee() internal pure returns (uint swapFee) {

        swapFee = uint(1000).sub(30);
    }
    ...
    function withdraw(uint8 v, bytes32 r, bytes32 s) public returns(bool){
        ...
        if (balance > 0){
            yBSCGToken.mint(msg.sender, balance);
    ...
    function setRewardToken(IyBSCGToken _rewardToken) public onlyOwner {
        require(address(_rewardToken) != address(0), "SwapMining: new RewardToken is the zero address");
        yBSCGToken = _rewardToken;
    }
